...
void DATATYPE_AnonymousArray_40::createFromStreamEx(MemoryStream& stream, TArray<int32>& datas)
{
	uint32 size = stream.readUint32();
	while(size > 0)
	{
...
void DATATYPE_AnonymousArray_40::addToStreamEx(Bundle& stream, const TArray<int32>& v)
{
	stream.writeUint32((uint32)v.Num());
	for(int i=0; i<v.Num(); ++i)
	{
